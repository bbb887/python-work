...
def greedy_vrp_multi():
    ...
    num_vehicles = int(info["VEHICLE_NUMBER"])
    ...
    total_cost = (total_distance * DISTANCE_WEIGHT
                  + total_time_penalty
    ...
    metrics = {
        "指标": [
            "总成本", "总飞行距离", "原始距离(无惩罚)", "时间惩罚", "约束惩罚",
            "Makespan(最长任务时间)", "超时订单数", "约束违规数", "使用的无人机数",
            "总服务客户数", "平均每架服务客户数", "最多服务客户数", "最少服务客户数",
            "算法运行时间(秒)"
        ],
        "数值": [
            f"{total_cost:.2f}", f"{total_distance:.2f}",
            f"{total_distance_no_penalty:.2f}", f"{total_time_penalty:.2f}",
            f"{total_constraint_penalty:.2f}", f"{makespan:.2f}",
            f"{overdue_count}", f"{constraint_violations}",
            f"{len(non_empty_routes)}", f"{len(customers)}",
