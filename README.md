...
├──data                          # 包含所有需要调用的清洗过后的csv文件，需要将csv与各个代码放在同一文件夹下
│
├── ____customers.csv           # 客户数据（由预处理生成，或直接提供）
├── ____info.csv                # 问题参数（车辆数、容量等）
│
...
### 步骤1：数据集及运行求解算法
将上述CSV 文件（客户、信息）拷贝到各算法脚本所在目录 **或** 修改脚本头部的文件路径变量。  
然后执行：
```bash
python 贪心算法改(1).py      # 贪心算法
python 遗传算法改(1).py      # 遗传算法
python 混合遗传算法改(1).py  # 混合遗传算法
python 退火算法改.py        # 模拟退火
```
...
所有改进版算法均读取以下参数，可通过修改该文件或脚本内全局变量调整：
